...
import { aiCostEstimate, type CostEstimate } from "../../../ipc/ai";
import {
...
const buildRangeTitle = (
	...
	const lines = covering.map((span, index) => {
		const labels = span.tagIds.map(
			(tagId) => tagLabelById.get(tagId) ?? `#${tagId}`,
		);
...
const SegmentEditor = ({
	...
	const submitNewSpeaker = async () => {
		...
		setError(null);
		try {
			const created = await speakerCreate(interviewId, {
				labelRaw: label,
				displayName: null,
			});
...
export const TranscriptViewer = ({
	...
	const tagLabelById = useMemo(
		() =>
			new Map(
				Array.from(tagMetaById.entries()).map(([tagId, meta]) => [
					tagId,
					meta.label,
				]),
			),
	...
	const requestTranscription = async () => {
		try {
			...
			setEstimate(
				await aiCostEstimate("transcribe", { interview_id: interviewId }),
			);
		} catch (e) {
			window.alert(errorMessage(e));
		}
			...
			>
				{segments.map((s, idx) => {
					...
					const isPlayingSegment =
						isActiveSegment && segmentPlaybackState.playing;
					const isLoopEnabled = Boolean(
						segmentPlaybackState.loopBySegmentId[s.id],
					);
								...
								<span className={styles.text}>
									{ranges.map((r, i) => {
										...
										const stripeColors = covering
											...
											.map(
												(tagId) => tagMetaById.get(tagId)?.color ?? "#5b9aff",
											);
										...
										return (
											<mark
												...
												data-overlap-count={
													covering.length > 1 ? covering.length : undefined
												}
												onClick={
													activeSpan
														? (e) => {
																...
																const nextSpan =
																	currentIndex >= 0
																		? covering[
																				(currentIndex + 1) % covering.length
																			]
																		: covering[0];
																setSelectedSpan(nextSpan.spanId);
																window.dispatchEvent(
																	new CustomEvent("workspace:open-right-pane"),
																);
															}
														: undefined
												}
											...
											>
												{slice}
												{hasSuggested && (
													<sup className={styles.aiBadge}>AI</sup>
												)}
											</mark>
										);
									})}
