...
pub const DEFAULT_PRETAG: &str = "Given the provided interview transcript and codebook, identify spans of text that should be supplemented with existing codes from the codebook. Only propose tags that are supported by the literal text. Use only existing tags from the provided codebook; do not invent or rename tags, and do not suggest new ones. Be conservative. Avoid unnecessary duplication: do not propose the same tag twice for the same passage, and do not create overlapping near-duplicate suggestions when one clear suggestion is enough. If no existing code clearly applies to a passage, do not propose anything for it. For each span, return the segment id, character offsets within that segment, and the tag(s) you propose.";
