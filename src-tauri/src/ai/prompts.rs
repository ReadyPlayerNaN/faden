...
pub const DEFAULT_CODEBOOK_GEN: &str = "You are assisting a researcher conducting reflexive thematic analysis (Braun & Clarke, 2006; 2022) of in-depth interviews.\n\nRead the following interview(s). Identify segments of text that carry analytically meaningful content — do not code everything, leave irrelevant passages uncoded. For each relevant segment, generate a code: a succinct label capturing a single distinct meaning. Codes should emerge from the data; do not impose a predetermined categorical scheme. Codes can range from semantic (close to the participant's own language) to latent (more interpretive or conceptual), depending on what the data warrants. Do not bundle multiple meanings into one code.\n\nEach code must include:\n- a short definition (1–2 sentences) describing the meaning or concept the code captures\n- 1–3 evidence quotes from the interview text\n\nReturn a flat list of codes as JSON matching the provided schema. Do not invent codes unsupported by the text.\n\nIf an existing codebook is provided, treat it as the starting point. Do not recreate existing codes, do not propose near-duplicates or semantically equivalent codes with different wording, and prefer reusing or broadening an existing code over adding a redundant new one. Only propose genuinely new codes when they add distinct analytical value beyond the existing codebook.\n\nTranscripts:\n{{transcripts}}\n\nExisting codebook:\n{{existing_codebook}}";

pub const DEFAULT_PRETAG: &str = "Given the provided interview transcript and codebook, identify spans of participant text that should be supplemented with existing codes from the codebook. Interviewer turns are included only as context; do not propose tags on interviewer segments. Only propose tags that are supported by the literal text. Use only existing tags from the provided codebook; do not invent or rename tags, and do not suggest new ones. Be conservative. Prefer the smallest complete relevant clause or subsentence, not a random phrase fragment, and avoid offsets that start or end mid-thought. If a suggested span overlaps an existing same-code span and clearly extends its relevant coverage, prefer proposing the broader covered range rather than a smaller overlapping duplicate. If no existing code clearly applies to a passage, do not propose anything for it. For each span, return the segment id, character offsets within that segment, and the tag(s) you propose.";
