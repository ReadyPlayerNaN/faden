...
fn estimates_text_tokens_with_tokenizer() {
    ...
    let e = cost::estimate("gemini-3-flash-preview", prompt, 100);
...
fn larger_prompt_costs_more() {
    let a = cost::estimate("gemini-3-flash-preview", "short", 100);
    let b = cost::estimate(
...
fn transcription_estimate_includes_audio_and_chunk_repeated_prompt_cost() {
    ...
    let expected_audio_tokens = (audio_seconds * 25.0).round() as u32;
    assert_eq!(
